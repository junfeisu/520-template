...
import plupload from './plupload.min.js'
...
function QiniuJsSDK() {
    ...
    this.uploader = function(op) {
        ...
        var reset_chunk_size = function() {
            ...
            // case IE 9-ï¼chunk_size is not empty and flash is included in runtimes
                ...
                if (file) {
                    switch (err.code) {
                        case plupload.FAILED:
                            errTip = 'ä¸ä¼ å¤±è´¥ãè¯·ç¨ååè¯ã';
                            break;
                        case plupload.FILE_SIZE_ERROR:
                            var max_file_size = up.getOption && up.getOption('max_file_size');
                            max_file_size = max_file_size || (up.settings && up.settings.max_file_size);
                            errTip = 'æµè§å¨æå¤§å¯ä¸ä¼ ' + max_file_size + 'ãæ´å¤§æä»¶è¯·ä½¿ç¨å½ä»¤è¡å·¥å·ã';
                            break;
                        case plupload.FILE_EXTENSION_ERROR:
                            errTip = 'æä»¶éªè¯å¤±è´¥ãè¯·ç¨åéè¯ã';
                            break;
                        case plupload.HTTP_ERROR:
                            if (err.response === '') {
                                // Fix parseJSON error ,when http error is like net::ERR_ADDRESS_UNREACHABLE
                                errTip = err.message || 'æªç¥ç½ç»éè¯¯ã';
                            ...
                            switch (err.status) {
                                case 400:
                                    errTip = "è¯·æ±æ¥ææ ¼å¼éè¯¯ã";
                                    break;
                                case 401:
                                    errTip = "å®¢æ·ç«¯è®¤è¯ææå¤±è´¥ãè¯·éè¯ææäº¤åé¦ã";
                                    break;
                                case 405:
                                    errTip = "å®¢æ·ç«¯è¯·æ±éè¯¯ãè¯·éè¯ææäº¤åé¦ã";
                                    break;
                                case 579:
                                    errTip = "èµæºä¸ä¼ æåï¼ä½åè°å¤±è´¥ã";
                                    break;
                                case 599:
                                    errTip = "ç½ç»è¿æ¥å¼å¸¸ãè¯·éè¯ææäº¤åé¦ã";
                                    if (!unknow_error_retry(file)) {
                                        return;
                                    }
                                    break;
                                case 614:
                                    ...
                                    break;
                                case 631:
                                    errTip = "æå®ç©ºé´ä¸å­å¨ã";
                                    break;
                                case 701:
                                    errTip = "ä¸ä¼ æ°æ®åæ ¡éªåºéãè¯·éè¯ææäº¤åé¦ã";
                                    break;
                                default:
                                    errTip = "æªç¥éè¯¯ã";
                                    if (!unknow_error_retry(file)) {
                            ...
                            errTip = errTip + '(' + err.status + 'ï¼' + errorText + ')';
                            break;
